...
<div class="page-header" style="display:flex;align-items:flex-start;justify-content:space-between">
    ...
    <button class="btn btn-primary" onclick="openModal('modalAdd')"><i class="fas fa-plus"></i> Tambah Resource</button>
</div>
...
<div class="stats-grid section">
    ...
    @foreach(['Human'=>['icon'=>'user','color'=>'var(--accent-sky)'],'Material'=>['icon'=>'box','color'=>'var(--accent-orange)'],'Equipment'=>['icon'=>'tools','color'=>'var(--accent-teal)']] as $tipe=>$cfg)
    @php $g = $summary['by_tipe'][$tipe] ?? ['count'=>0,'total_biaya'=>0]; @endphp
    <div class="stat-card">
        <div class="stat-label" style="color:{{ $cfg['color'] }}"><i class="fas fa-{{ $cfg['icon'] }}"></i> {{ $tipe }}</div>
...
<div class="card">
    ...
    <div class="table-wrap">
        <table>
            <thead><tr><th>Nama Resource</th><th>Tipe</th><th>Task</th><th>Jumlah</th><th>Satuan</th><th>Biaya/Satuan</th><th>Total Biaya</th><th></th></tr></thead>
            <tbody>
            ...
            <tr>
                ...
                <td style="font-size:13px;font-weight:600;color:var(--primary)">Rp {{ number_format($r->total_biaya,0,',','.') }}</td>
                <td>
                    <div style="display:flex;gap:4px">
                        <button class="btn btn-icon" onclick='openEdit({{ $r->toJson() }})' title="Edit"><i class="fas fa-pencil"></i></button>
                        <form method="POST" action="{{ route('projects.resources.destroy',[$project,$r]) }}" onsubmit="return confirm('Hapus resource ini?')">
                            @csrf @method('DELETE')
